    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users WHERE email='{$email}'")) > 0) {
        ...
        if ($query) {        
            ...
            try {
                ...
                $mail->Username   = 'aikojosephines@gmail.com';                     //SMTP username
                $mail->Password   = 'bnyibayvlzwzaqip';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('aikojosephines@gmail.com');
                $mail->addAddress($email);

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'no reply';
                $mail->Body    = 'Here is the verification link <b><a href="http://localhost/ShoeWebsite/changepassword.php?reset='.$code.'">http://localhost/ShoeWebsite/changepassword.php?reset='.$code.'</a></b>';
